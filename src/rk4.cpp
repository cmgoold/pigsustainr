...
std::vector<double> RK4::integrate(){
  ...
  //constants
  double constant_ = 1.0/6.0;
  double one_half_ = 0.5;

  // step 1
  std::vector<double> step_k_1_ = model_->derivatives(states_);
  ...
  std::vector<double> step_k_2_ = model_->derivatives(step_2_states_);
  for(int k = 0; k < n_states_; ++k){
  ...
  std::vector<double> step_k_3_ = model_->derivatives(step_3_states_);
  for(int k = 0; k < n_states_; ++k){
  ...
  std::vector<double> step_k_4_ = model_->derivatives(step_4_states_);
  for(int k = 0; k < n_states_; ++k){
  ...
  for(int k = 0; k < n_states_; ++k){
    new_states_.push_back(states_[k] + constant_ * (K1[k] + (K2[k] + K3[k]) * 1/one_half_ + K4[k] ) );
  }
